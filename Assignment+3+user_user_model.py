...
matrices_user = sio.loadmat('folds_user_cold_start.mat')
...
train_ratings=[]
train_features=[]
for x in range(0,len(train_ids)):
...
for x in range(10,110,10):
    ...
    elm.train(np.asarray(train_features),np.asarray(train_ratings), "LOO")
    Preds = elm.predict(np.asarray(test_features))
    predicted_labels = np.argmax(Preds, axis=1)
    true_labels = np.argmax(test_ratings, axis=1)
    print mae(true_labels,predicted_labels)/4
